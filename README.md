...
- eat the dots, get +10 points, watch a little popup float up when you do
- 3 lives, lose them all and its game over
- high score gets saved in `localStorage` so it survives a refresh
...
- scrolling cyan grid in the background (its actually animated now, took me embarrassingly long to catch that typo)
- vignette around the edges so your eyes go to the board
- neon glow around the board that slowly pulses + cycles through colors
- CRT style scanlines over the board
- subtle screen flicker like an old monitor
- neon "PAC-MAN" title up top that flickers on like a sign warming up, plus a lil wiggle
- blinking "PRESS ARROW KEYS TO START" text
